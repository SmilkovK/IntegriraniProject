...
namespace Project_2.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
            ...
            IEmailSender emailSender)
        {
            ...
            _emailStore = (IUserEmailStore<DeliveryUser>)GetEmailStore();
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                await _userStore.SetUserNameAsync((DeliveryUser)user, Input.Email, CancellationToken.None);
                await _emailStore.SetEmailAsync((DeliveryUser)user, Input.Email, CancellationToken.None);
                var result = await _userManager.CreateAsync((DeliveryUser)user, Input.Password);

                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");

                    var userId = await _userManager.GetUserIdAsync((DeliveryUser)user);
                    var code = await _userManager.GenerateEmailConfirmationTokenAsync((DeliveryUser)user);
                    ...
                    {
                        await _signInManager.SignInAsync((DeliveryUser)user, isPersistent: false);
